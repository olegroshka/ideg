...
infinite-time average misses the time-averaged metric by 38--55\%
(class medians; 8\% metastable), because mutual information is
...
\item \textbf{A non-commutation result.} Time averaging and metric
  ...
  the time-averaged metric by 38--55\% (class medians; 8\% for the
  metastable class), and the mismatch is neither a finite-window
...
removing exactly the motion moves the metric by $38$--$55\%$ (class
medians) in five of six classes, and by $8\%$ in the metastable class,
whose time-averaged metric is nearly carried by its own infinite-time
average.
...
pinning barely costs ($0.0086 \to 0.030$, 20/20). \textbf{The
...
$0.0052$ (20/20) & \SECTORTWO \\
\;\;+ sector weights pinned to the run (T3) & $\leq N$ (eff.) &
$0.244$ (15/20) & \SECTORTHREE \\
\bottomrule
\end{tabular}
...
by 38--55\% in five of six classes, window-length effects excluded);
