...
commit-hashed record; six claim-level corrections, each caught by the
study's own audit mechanisms, are reported as results.
\end{abstract}
...
search then matched; and two successive adversarial audits of the
comparator search each forced it open further --- first to
unrestricted diagonal populations, which overturned our remaining
existential claim, then to stationary states with coherences inside
degenerate energy blocks, which matched the last resisting class
almost exactly. What survives this gauntlet is correspondingly
compact:
\begin{enumerate}
...
misses remain found optima, not certified minima. Sparse supports
re-run the
