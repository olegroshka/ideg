...
populates, and restricting to the dynamically accessible sector
collapses the match ($\sim 0.24$). Within the stationary algebra its
own dynamics licenses, no faithful impostor of the quasiperiodic
metric was found. Third, weak dephasing \emph{pins} the moving
...
\item \textbf{An admissibility-resolved matching spectrum.} Whether a
  time-averaged metric has a faithful stationary impostor depends on
  ...
  restricted to the dynamically accessible sector, the match
  collapses to the population-only plateau ($\sim 0.24$). Two
  invariant class discriminators survive: the
  \emph{smooth-in-energy representation gap}, and \emph{sector
  admissibility} --- no faithful impostor of the quasiperiodic metric
  was found inside the stationary algebra its own dynamics licenses.
...
run's own $\mathrm{Tr}(\rho\,\Pi_q)$ --- the dynamically accessible
stationary states, since XX evolution conserves $N_{\rm mag}$. Every
...
runs whose faithful impostor lives \emph{entirely} outside the
one-magnon sector); integrable T3 stays matched ($0.012$, 20/20).
\textbf{The
near-exact quasiperiodic impostor exists only outside the dynamically
accessible sector: within the stationary algebra the dynamics itself
licenses, no faithful impostor of the quasiperiodic metric was found.}
The class-resolved resource is not coherence but \emph{admissibility}
--- which stationary algebra the impostor is allowed to live in.
...
weights to the run's own --- the dynamically accessible stationary
states --- and for a one-magnon state this reduces to diagonal
populations on the $N$ nondegenerate magnon levels.}
\label{tab:price}
\end{table*}
...
commutant succeeds, and the dynamically accessible subalgebra fails
again --- so which admissibility criterion is the physically honest
...
matches T1 with a third of its parameters, and T3 fails with the
most physical constraint of all. Conversely, the chaotic classes'
...
lives on magnetization sectors the dynamics never populates, and
within the stationary algebra the dynamics itself licenses, no
faithful impostor of the quasiperiodic metric was found. What the
...
whether a faithful impostor exists is governed by the
...
metric is reproduced only from outside the dynamically accessible
sector; and weak noise stabilizes exactly the class whose metric no
admissible stationary state represents. ``Motion-borne'' thus
survives --- not as an unconditional existence claim, but as a
measured statement relative to the physically honest algebra.
...
dynamically accessible sector --- and is there a principled measure
...
with a third of its parameters; the accessible tier fails with the
fewest and most physical constraints), and how these gaps scale with
