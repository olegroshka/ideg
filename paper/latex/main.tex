...
matched versus threshold for the smooth, sector-pinned (T3), and
full-commutant (T1) searches, both sizes. The registered $\ephi =
0.25$ (vertical line) sits in the steep region of the T3 curves ---
which is why T3 counts at that threshold straddle --- while the T1
curves saturate two orders of magnitude below it: the tier gaps, not
the counts, are the threshold-insensitive observables.}
...
--- which stationary algebra the impostor is allowed to live in. We
do not discount the threshold counts: at the registered $\ephi =
0.25$, T3 impostors qualify in 15/20 runs at $N{=}10$ and 9/20 at
$N{=}12$. But two measured facts fix what such qualification means.
$\ephi$ was calibrated as the metric construction's finite-size noise
floor, and the quasiperiodic metric's own motion exceeds it: the
typical instantaneous distance $\lVert D(t)-\bar D\rVert/\lVert\bar
D\rVert$ is $0.26$--$0.33$ (per-run medians) --- \emph{larger} than
the best T3 miss. A threshold-level ``match'' is therefore a match at
the scale of the motion's own excursions, where the criterion stops
resolving anything; the commutant match, at $0.003$--$0.005$, is a
reproduction fifty times below the floor. Fig.~\ref{fig:comparator}(c)
makes the point without a threshold choice: the T3 curves rise
steeply \emph{through} $\ephi$ (counts there are maximally
sensitive), the T1 curves saturate far below it, and the gap between
them is threshold-insensitive.
